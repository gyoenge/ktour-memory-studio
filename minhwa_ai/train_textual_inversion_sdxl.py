...
def parse_args():
    ...
    # --- ✨ 여기에 lora_model_path 인자를 추가합니다 ---
    parser.add_argument(
        "--lora_model_path",
        type=str,
        default=None,
        help="Path to the LoRA weights folder that should be loaded before training.",
    )
    # ----------------------------------------------------
    parser.add_argument(
        "--revision",
...
def main():
    ...
    # --- ✨ 이 부분을 아래와 같이 수정합니다 ---
    if args.lora_model_path: # lora_model_path 인자가 주어졌을 경우에만 실행
        print(f"Loading pretrained LoRA weights from {args.lora_model_path}")
        unet.load_attn_procs(args.lora_model_path)
        print(f"Successfully loaded LoRA weights into UNet.")
    # ------------------------------------
